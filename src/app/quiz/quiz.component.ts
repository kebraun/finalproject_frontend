...
export class QuizComponent implements OnInit {
  ...
  getCatBreedDetails = (quizForm: NgForm) => {
    ...
    let intelligence = parseInt(quizForm.value.intelligence);
    // let friendLevel = parseInt(quizForm.value.friendly);

    console.log(`playLevel:${playLevel} vocalLevel:${vocalLevel} 
		 shedLevel:${shedLevel} socialLevel:${socialLevel} intelligenceLevel${intelligence}`);

    //  strangerLevel:${friendLevel}
    ...
    let filteredBreeds = this.catBreeds.filter((breed) => {
      return (
        ...
        this.getLevelsToCheck2(socialLevel).includes(breed.social_needs) &&
        this.getLevelsToCheck2(intelligence).includes(breed.intelligence)
        // &&
  ...
  getLevelsToCheck2 = (selectedOption: number): number[] => {
    let levelsToCheck: number[] = [];
    if (selectedOption === 0) {
      levelsToCheck = [3, 4, 5];
  ...
  toggleDisplayIntell = () => {
    this.showDivIntell = !this.showDivIntell;
  };
}
